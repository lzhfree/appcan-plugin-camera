...
-(void)open:(NSMutableArray *)inArguments {
    ...
    if ([inArguments isKindOfClass:[NSMutableArray class]] && [inArguments count]>0) {
        NSLog(@"hui-->uexCamera-->open inArguments = %@",inArguments);
...
-(void)showCamera {
    NSLog(@"hui-->uexCamera-->showCamera");
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
	...
	if ([mediaType isEqualToString:@"public.image"]){
        ...
        NSLog(@"hui-->uexCamera-->image orientation = %d",image.imageOrientation);
...
-(NSString *)getSavename:(NSString *)type {
	...
	NSLog(@"hui-->uexCamera-->wgtpath = %@",wgtPath);
...
-(UIImage *)imageByScalingAndCroppingForSize:(UIImage *)sourceImage width:(float)destWith {
	...
	NSLog(@"hui-->uexCamera-->src w = %f,src h = %f",srcWidth,srcHeight);
	CGFloat targetWidth;
	...
	NSLog(@"hui-->uexCamera-->after:tarw= %f,tarh = %f",targetWidth,targetHeight);
	...
	NSLog(@"hui-->uexCamera-->scalew = %f scaleh = %f",scaledWidth,scaledHeight);
...
-(void)savaImg:(UIImage *)image {
	...
	NSLog(@"hui-->uexCamera-->save image path = %@", imagePath);
	...
	NSLog(@"hui-->uexCamera-->save image size = %d",imageData.length);
